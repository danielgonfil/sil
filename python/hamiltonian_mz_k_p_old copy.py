...
def build_basis_mz_k_p(N, mz, k, p):
    ...
    for s in basis_mz:
        ...
        for sigma in [+1, -1]:
            ...
            if m != -1:
                km = 2 * np.pi * k * m / N
                if np.isclose(1 + sigma * p * np.cos(km), 0.0, atol=1e-10):
                    R = -1
                if sigma == -1 and not np.isclose(1 - sigma * p * np.cos(km), 0.0, atol=1e-10):
                    R = -1

            if R > 0:
                print('basis', m, 1 + sigma * p * np.cos(km), 1 - sigma * p * np.cos(km))
                basis.append(s)
...
def helement(a, b, l, q, k, p, Ra, ma, sigma):
    ...
    if sigma_a == sigma_b: # 151
        if m == -1:
            ...
            print(cos_kl + sigma_a * p * cos_kl_m, 1 + sigma_a * cos_km)
...
if __name__ == "__main__":
    ...
    for k in range(-N // 2 + 1, N // 2 + 1):
        for p in [+1, -1]:
            print(f"Calculating for k = {k}, p = {p}")
            H = hamiltonian_mz_k_p(N, mz, k, p)
            eigenvalues, eigenvectors = np.linalg.eig(H)
