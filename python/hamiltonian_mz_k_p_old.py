...
def helement(a, b, l, q, k, p, Ra, ma, sigma, N):
    k = k % N
    
    # equations 151 and 152 from sandvik
    sigma_a = sigma[a]
    sigma_b = sigma[b]
    # print(f"helement called with a={a}, b={b}, sigma_a={sigma_a}, sigma_b={sigma_b}, l={l}, q={q}")
    ...
    if sigma_a == sigma_b: # 151
        if q == 0:
            ...
            print(cos_km, sigma_a, p)
            print(1 + sigma_a * cos_km)
            factor = (cos_kl + sigma_a * p * cos_kl_m) / (1 + sigma_a * p * cos_km)
    else: # 152
        if q == 0:
            ...
            factor = (-sigma_a * sin_kl + p * sin_kl_m) / (1 - sigma_a * p * cos_km)

    # print("Computed value:", hj * sigma_a * norm_factor * factor)
...
def hamiltonian_mz_k_p(N, mz, k, p):
    ...
    M = len(basis) # M ≠ 2 ** N
    H = np.zeros((M, M), dtype=float) # this is not complex !!!

    state_idx = 0
    while state_idx < M: # looping over all states
        state = basis[state_idx]
        # print("\nProcessing state:", state, "at index", state_idx)
        ...
        for i in range(N): # looping over all spins to flip two adjacent spins
            ...
            if get_spin(state, i) != get_spin(state, j):
                ...
                if state_flip_rep_idx >= 0:
                    if state_flip_rep_idx < M - 1 and basis[state_flip_rep_idx + 1] == state_flip_rep: 
                        m = 2 # do that again
                    else: 
                        m = 1
                    ...
                    for j_block in range(m):
                        for i_block in range(n):
                            ...
                            H[ai][bi] += helement(ai, bi, l, q, k, p, R_list, m_list, sigma_list, N)

                # if state_flip_rep_idx >= 0:
                #     if state_flip_rep_idx > 0 and basis[state_flip_rep_idx - 1] == state_flip_rep:
                #         m = 2
                #         state_flip_rep_idx = state_flip_rep_idx - 1
                #     if state_flip_rep_idx < M - 1 and basis[state_flip_rep_idx + 1] == state_flip_rep: 
                #         m = 2 # do that again
                #     else: 
                #         m = 1

                #     for j_block in range(state_flip_rep_idx, state_flip_rep_idx + m):
                #         for i_block in range(state_idx, state_idx + n):
...
if __name__ == "__main__":
    ...
    #     for p in [+1, -1]:
    #         basis, R_list, m_list, sigma_list = build_basis_mz_k_p(N, 0, k, p)
    #         print(f"k={k}, p={p}, basis size={len(basis)}")
    ...
    for k in range(-N // 2 + 1, N // 2 + 1):
        for p in [+1, -1]:
            H = hamiltonian_mz_k_p(N, mz, k, p)
            eigenvalues, eigenvectors = np.linalg.eig(H)
            print(f"Eigenvalues for k = {k}, p = {p}", np.real(np.round(np.sort(eigenvalues), 4))) # even if matrix is complex it is hermitian so the eigenvalues are real
